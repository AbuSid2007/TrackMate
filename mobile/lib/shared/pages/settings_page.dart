...
class _SettingsPageState extends State<SettingsPage> {
  ...
  @override
  void dispose() {
    for (final c in [_bioCtrl, _heightCtrl, _weightCtrl, _stepGoalCtrl, _calorieGoalCtrl]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  // 🔥 View existing application bottom sheet
// 🔥 View existing application bottom sheet (UPDATED)
  ...
  Future<void> _showTrainerApplicationDialog(BuildContext context) async {
    ...
    bool _autoValidate = false; // 🔥 Validation stays silent until first submit!
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (BuildContext context, StateSetter setModalState) {
          ...
          return Padding(
            ...
            child: Form(
              key: formKey,
              // 🔥 Only triggers red text AFTER they hit submit once
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (_autoValidate && specializations.isEmpty)
                      const Padding(
                    ...
                    if (_autoValidate && certifications.isEmpty)
                      const Padding(
                        padding: EdgeInsets.only(top: 4, left: 12),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          // 🔥 If validation fails, turn on auto-validation so fields turn red
  ...
  Widget _inputTile(String label, TextEditingController ctrl, {int maxLines = 1}) =>
      Padding(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
        child: TextField(controller: ctrl, maxLines: maxLines, decoration: InputDecoration(labelText: label)),
      );
  ...
  Widget _dropdownTile(String label, String? value, List<String> items, ValueChanged<String?> onChanged) =>
      Padding(
        ...
        child: DropdownButtonFormField<String>(
          ...
          items: items.map((i) => DropdownMenuItem(value: i, child: Text(_capitalize(i)))).toList(),
          onChanged: onChanged,
        ),
      );
